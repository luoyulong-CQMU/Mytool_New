...
class ReportCard(object):
    ...
    def format_pain_message(self):
        ...
        if len(self.pain_in_time) <= 6:
            ...
            print("\n-----------------------------------\n")
            if len(self.diag1) == 0:
                print("Error:你似乎未完成首页填写，或填写后未刷新界面，请重试\n")
            else:
                pass
    ...
    def report_card(self):
        ...
        select = input("即将开始打印，请放入纸张，并确认信息是否正确？ [Y/n]: ")
        if select == "n" or select == "no" or select == "N":
            pass
        else:
            print("*" * 50)
            print("开始设置文件，调用系统默认打印机")
            print_pain_info()
            print("文件已传输至默认打印机，任务完成，请等待打印")
            print("*" * 50)

        # try:
        #     quality = input('是否继续打印病历质量信息？[Y/n]: ')
        #     if quality == 'y' or quality == 'Y':
        #         draw_quality(self.pain_name, self.pain_id)
        #         print("即将开始打印,请放入纸张，并确认信息是否正确？")
        #         os.system("pause")
        #         print_pain_info()
        #         print("打印已完成")
        #         print("----------------------------------")
